...
class _DeviceDetailsScreenState extends State<DeviceDetailsScreen> {
  ...
  void initState() {
    ...
    // Auto-scroll chart to the end on load
  ...
  Future<void> _runDeepScan() async {
    ...
    try {
      ...
      // -O: OS detection
      // -T4: Faster execution
      process = await Process.start('nmap', [
        '-sV',
        '-O',
        '-T4',
      ...
      process.exitCode.then((code) async {
        ...
        if (code == 0) {
          ...
          lines.add(
            'Note: OS detection (-O) usually requires root/sudo (Linux) or Administrator (Windows) privileges.',
          );
        }
        emit('DONE');
        await closeStream();
      ...
      lines.add(
        '[ERROR] "nmap" utility not found. Please install nmap to use this feature.',
      );
      emit('ERROR');
      await closeStream();
    }
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, i) => Text(
                  ...
                  style: GoogleFonts.jetBrainsMono(
                    color: lines[i].startsWith('[SYSTEM ERR]')
                        ? Colors.redAccent
  ...
  Future<void> _runTraceroute() async {
    ...
    try {
      process = await Process.start(cmd, [widget.device.address],
          runInShell: Platform.isWindows);
      stdoutSub = process.stdout
          .transform(const Utf8Decoder(allowMalformed: true))
          .transform(const LineSplitter())
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            _buildStabilityHeader(
              context,
              widget.device.stabilityScore,
  ...
  Widget _buildStabilityHeader(BuildContext context, double score) {
    ...
    Color scoreColor = score >= 90
        ...
        : (score >= 70 ? const Color(0xFFF59E0B) : const Color(0xFFEF4444)); // Amber / Red
        ...
        Row(
          children: [
            ...
            Expanded(
              child: _buildStatCard(
                'PACKET LOSS',
                '${stats.avgLoss.toStringAsFixed(0)}%',
                Icons.leak_add,
              ),
            ),
          ],
        ...
        Row(
          children: [
            Expanded(
              child: _buildStatCard(
                'PEAK JITTER',
                '${stats.maxLatency.toStringAsFixed(1)}ms',
                Icons.trending_up,
              ),
            ),
            const SizedBox(width: 16),
  ...
  Widget _buildChartSection(BuildContext context, _DeviceStats stats) {
    ...
    // Ensure we have at least 30px per point
                        ...
                        titlesData: FlTitlesData(
                          ...
                          bottomTitles: AxisTitles(
                            ...
                            sideTitles: SideTitles(
                              ...
                              getTitlesWidget: (v, m) {
                                if (v.toInt() >= 0 &&
                                    v.toInt() < widget.device.history.length) {
                                  final time = widget
                                      .device
                                      .history[v.toInt()]
                                      .timestamp;
                              ...
                              return touchedSpots.map((spot) {
                                if (spot.x.toInt() >= 0 &&
                                    spot.x.toInt() <
                                        widget.device.history.length) {
                                  final historyItem =
                                      widget.device.history[spot.x.toInt()];
              ...
              return Row(
                children: [
                  Icon(
                    Icons.circle,
                    size: 8,
                    color: h.status == DeviceStatus.online
                        ? Colors.green
                        : Colors.red,
                  ),
                  const SizedBox(width: 12),
  ...
  _DeviceStats _calculateStats() {
    ...
    final online = widget.device.history
        .where((h) => h.status == DeviceStatus.online)
        .toList();
    ...
    for (var h in widget.device.history) {
      ...
      if ((h.latencyMs ?? 0) < minLatency && h.status == DeviceStatus.online)
